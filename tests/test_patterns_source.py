...
The Sherr-I tab was showing sqlite demo rows that looked exactly like real insights.
...
def test_no_sprie_naming_remains():
    """Rename completed: SPRIE -> Sherr-I everywhere."""
...
def test_postgres_is_tried_before_engine_url():
    ...
    assert CODE.index("if SHERR_I_DATABASE_URL:") < CODE.index("if ENGINE_URL:")


def test_spie_db_url_falls_back_to_database_url():
    assert 'os.getenv("SHERR_I_DATABASE_URL") or os.getenv("DATABASE_URL")' in CODE
